...
const cart: FastifyPluginAsyncJsonSchemaToTs = async (app) => {
	...
	app.addHook("preHandler", async (req) => {
		...
		if (!req.profile_id) throw new UnauthorizedAccess("Requires profile setup");
	});
	...
	app.route({
		...
		handler: async (req) => {
			await CartService.assertGroupMember(req.params.groupId, req.profile_id!);
	...
	app.route({
		...
		handler: async (req) => {
			await CartService.assertGroupMember(req.params.groupId, req.profile_id!);
			return CartService.removeItem(req.params.groupId, req.body.id);
		},
	});
